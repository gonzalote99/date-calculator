...
function validateDate(startDate, endDate) {
  if(startDate > endDate) 
    throw new Error('start date cannot higher than end')
...
function getDateDiffStr(startDate, endDate) {
  ...
  const monthsLeftStr = `${Math.floor(monthsLeft)} month${Math.floor(monthsLeft) > 1 ? 's' : ''}`;
...
function createResultElement({dateDiffStr, ...dateDiff}) {
  ...
  resultEL.textContent = `result: ${formatNumber(dateDiff.days)} day${dateDiff.days > 1 ? 's' : '' }`;

  col1El.appendChild(resultEL);

  const resultParaEl = document.createElement('p');
  resultParaEl.textContent = `it is ${formatNumber(dateDiff.days)} day ${dateDiff.days > 1 ? 's' : ''} no including end date `;
  ...
  resultContainerEl.textContent = `(OR) ${dateDiffStr}`;
  ...
  alternativeHeadingEl.textContent = `alternative units`;
  col2El.appendChild(alternativeHeadingEl);

  const alternativeParaEl = document.createElement('p');
  alternativeParaEl.textContent = `${formatNumber(dateDiff.days)} day ${dateDiff.days > 1  ? 's' : ''}can be converted these units`;
  ...
  .forEach(([key, unit]) => {
    ...
    unitsListItem.textContent = `${formatNumber(unit)} ${unit > 1 ? key : key.slice(0, -1)}`;
    unitList.appendChild(unitsListItem);
  });

  col2El.appendChild(unitList);
...
function showPlaceHolderTextOnDOM() {
  ...
  placeholderEl.textContent = `enter dates to calculate`;
